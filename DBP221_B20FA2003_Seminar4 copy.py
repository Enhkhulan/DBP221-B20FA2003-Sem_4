...
Marcelo_G = [80,77,82,82,73,82,58,78,6,35]
SergioBusquets_G = [82,57,82,79,76,72,60,72,79,80]
Karim_G = [79,78,75,81,76,79,62,76,77,69]
LuisSuarez_G = [80,65,77,66,69,77,55,67,77,40]
...
DaniAlves_G = [75,51,51,79,77,76,49,69,54,62]

Games = np.array([[Marcelo_G], [SergioBusquets_G], [Karim_G], [LuisSuarez_G], [SergioAguerod_G], [FranckRibery_G], [ToniKroos_G], [CarlesPuyol_G], [ZlatanIbrahimovic_G], [DaniAlves_G]])

print("Баганын нийт тоглолт", np.sum( Games, axis = 0))

print("Мөрийн нийт тоглолт", np.sum( Games, axis = -1))

Marcelo_FG = [20,34,45,37,46,34,53,44,46,52]
SergioBusquets_FG = [38,50,42,45,46,30,31,49,41,47]
Karim_FG = [25,44,24,40,31,42,18,19,39,43]
LuisSuarez_FG = [18,32,26,37,20,35,34,46,24,19]
...
DaniAlves_FG = [30,44,50,35,26,41,18,28,40,43]

FieldGoals  = np.array([[AndreaPirlo_FG], [SergioBusquets_FG], [Karim_FG], [LuisSuarez_FG], [SergioAguerod_FG], [FranckRibery_FG], [ToniKroos_FG], [CarlesPuyol_FG], [ZlatanIbrahimovic_FG], [DaniAlves_FG]])
